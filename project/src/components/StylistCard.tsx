    ...
    <div className="bg-white rounded-xl shadow-md overflow-hidden transition-transform duration-300 hover:scale-[1.02] hover:shadow-lg">
      ...
      <div className="p-6">
        ...
        <button
          ...
          className={`w-full py-2 rounded-md transition-colors flex items-center justify-center ${stylist.available
            ? 'bg-purple-600 text-white hover:bg-purple-700'
            : 'bg-gray-200 text-gray-500 cursor-not-allowed'
            }`}
          disabled={!stylist.available}
        >
          {stylist.available ? 'Reserve com este estilista' : 'Atualmente indisponível'}
        </button>
      </div>
    </div>
  );
